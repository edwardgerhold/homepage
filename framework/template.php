...
class Template extends Base {
    ...
    protected function _tag($source) {
        ...
        if (isset($type["tags"])) {
            $tags = implode("|", array_keys($type["tags"])); // if|elseif|for|foreach
            $regex = "#^(/){0,1}({$tags})\s*(.*)$#"; // matches
        ...
        if ($tag && $closer) { // node des closer tags
            return array(
                "tag" => $tag,
                "delimiter" => $delimiter,
                "closer" => true,
        ...
        return array( // Node des Opener Tags
            "tag" => $tag,
            "delimiter" => $delimiter,
            "closer" => false,
            "source" => $extract,
    ...
    public function parse($template) {
        if (!is_a($this->implementation, "\Framework\Template\Implementation")) {
            throw new Exception("template implementation fails is_a");
        }
        $array = $this->_array($template);
        $tree = $this->_tree($array["all"]);
        $this->_code = $this->header . $this->_script($tree) . $this->footer;

        // echo "<pre>".htmlspecialchars($this->_code)."</pre>";
    ...
    public function process($data = array()) {
        if ($this->_function == null) {
            throw new Exception\Parser("got no generated function code");
        }
        try {
