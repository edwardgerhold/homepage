...
class Standard extends Template\Implementation {
    ...
    protected function _for($tree, $content) {
        ...
        $element = $tree["arguments"]["element"];
        return $this->_loop(
            $tree, "for ({$element}_i=0; {$element}_i < sizeof({$object}); {$element}_i++) { {$element}={$object}[{$element}_i]; {$content} }"
        );
    }
    ...
    protected function _macro($tree, $content) {
        ...
        $name = $arguments["name"];
        $args = $arguments["args"];
        return "function {$name} ({$args}) { \$_text=array(); {$content}; return implode(\$_text); }";
    }
    ...
    protected function _loop($tree, $inner) {
        $number = $tree["number"];
        $object = array();
        if (isset($tree["arguments"]["object"]))
            $object = $tree["arguments"]["object"];
